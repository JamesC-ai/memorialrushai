...
test("renders MemorialRushAI planner", async () => {
  ...
  assert.match(html, /Create free brief first/);
  assert.match(html, /Check paid fit/);
  assert.match(html, /Generate tribute brief/);
  assert.match(html, /Email order pack/);
  assert.match(html, /Use the free planner first/);
  assert.match(html, /Pay \$49 after fit/);
  assert.match(html, /Pay \$99 after fit/);
  assert.match(html, /Free tribute brief first/);
  assert.match(html, /\$49\/\$99 only after family fit/);
  assert.match(html, /No photo upload by default/);
  assert.match(html, /No delivery or approval guarantee/);
  assert.match(html, /Buy only when the handoff is ready/);
